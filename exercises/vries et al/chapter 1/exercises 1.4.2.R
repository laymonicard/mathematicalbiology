...
#### Using 1/4 day as the timestep ####
#### Discrete-time model ####
# Set up the time steps
timestep <- 1/4
n_length <- seq(0, 15, timestep)
...
# The solutions from both models are closer to each other compared to the one with 1/2 day time step


#### Using 1/8 day as the timestep ####
#### Discrete-time model ####
# Set up the time steps
timestep <- 1/8
n_length <- seq(0, 15, timestep)
...
# The solutions from both models are even closer to each other compared to the one with 1/2 day time step

#### Using 1 day as the timestep ####
#### Discrete-time model ####
# Set up the time steps
timestep <- 1
n_length <- seq(0, 15, timestep)
...
# The solutions are a bit farther from each other 


#### Using 2 days as the timestep ####
#### Discrete-time model ####
# Set up the time steps
timestep <- 2
n_length <- seq(0, 15, timestep)
...
# The solution gets even farther from each other at larger timestep


#### Using 10 days as the timestep ####
#### Discrete-time model ####
# Set up the time steps
timestep <- 10
n_length <- seq(0, 150, timestep)
...
# In this case, alpha = probability of recovery every one day / one day
# Find the value of p such that alpha for a half day remains 0.3
p <- 1-(1-0.3)^10 # probability of the event to happen at least once is 1 - the probability of the events not occuring at all in the whole 10 days
